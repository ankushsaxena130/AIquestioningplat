...
# Loads backend/.env into the process environment before anything below
# reads os.environ — so GEMINI_API_KEY (or XAI_API_KEY / OPENAI_API_KEY /
# DATABASE_URL / JWT_SECRET) can just live in a .env file next to this
# script instead of being exported manually every time you open a
# terminal. Does nothing (silently) if no .env file exists — real
# environment variables set some other way still work exactly as before.
load_dotenv()
...
if LLM_PROVIDER == "openai":
    ...
    # "grok-4" was retired May 2026 and now silently redirects to grok-4.3
    # — using the real current model name directly instead of relying on
    # that redirect.
...
# Sarvam AI — used only for Friday's voice (text-to-speech). Completely
# independent of LLM_PROVIDER above; Friday's brain (question logic) and
# Friday's voice (audio) are two separate concerns. If unset, the
# frontend automatically falls back to the browser's built-in speech
# synthesis, so voice still works either way — Sarvam just sounds better.
SARVAM_API_KEY = os.environ.get("SARVAM_API_KEY")
SARVAM_TTS_MODEL = os.environ.get("SARVAM_TTS_MODEL", "bulbul:v3")
SARVAM_TTS_SPEAKER = os.environ.get("SARVAM_TTS_SPEAKER", "priya")
...
class UserRegisterIn(BaseModel):
    email: str
    password: str
    # No role field — public self-registration always creates a client
    # account. Consultant access is a fixed credential (see /auth/login),
    # not something anyone can sign themselves up for.
...
def require_user(current_user: Optional[dict] = Depends(get_current_user)) -> dict:
    """Same as get_current_user, but rejects the request with 401 instead
    of silently returning None — used on endpoints that must be behind
    login (the consultant dashboard, submitting a session), as opposed to
    endpoints that only optionally personalize behavior when a token is
    present."""
    if not current_user:
...
# Fixed consultant credential — intentionally NOT a database record, and
# there is no registration path that can create it (see UserRegisterIn
# above). This is the one and only way to reach the consultant role.
CONSULTANT_LOGIN_ID = "spidey"
CONSULTANT_LOGIN_PASSWORD = "2026"


@app.post("/auth/register")
def register(payload: UserRegisterIn):
    if payload.email.strip().lower() == CONSULTANT_LOGIN_ID:
...
def login(payload: UserLoginIn):
    # Consultant login is a fixed id/password pair, checked before ever
    # touching the user database — there's no account to look up.
    if payload.email.strip().lower() == CONSULTANT_LOGIN_ID and payload.password == CONSULTANT_LOGIN_PASSWORD:
        consultant_user = {"id": "consultant-spidey", "email": CONSULTANT_LOGIN_ID, "role": "consultant"}
        token = create_access_token(consultant_user)
        return {"token": token, "user": consultant_user}

    user = _USERS.get_by_email(payload.email)
    if not user or not verify_password(payload.password, user["passwordHash"]):
        raise HTTPException(401, "Invalid email/ID or password")
    token = create_access_token(user)
    return {"token": token, "user": {"id": user["id"], "email": user["email"], "role": user["role"]}}
...
@app.post("/projects")
def create_project(project: ProjectIn, current_user: dict = Depends(require_user)):
...
@app.get("/projects")
def list_projects(current_user: dict = Depends(require_user)):
...
def get_project(project_id: str, current_user: dict = Depends(require_user)):
    project = _PROJECTS.get(project_id)
    if not project:
        raise HTTPException(404, "Project not found")
    if current_user["role"] == "client" and project.get("ownerId") != current_user["id"]:
        raise HTTPException(404, "Project not found")
    return project
...
class TTSIn(BaseModel):
    ...
    pace: Optional[float] = 1.05


@app.post("/voice/tts")
def synthesize_speech(payload: TTSIn):
    """
    Proxies text to Sarvam AI's Bulbul v3 model for natural-sounding
    speech — the browser's built-in speechSynthesis voices are what
    Friday used before this, and they sound noticeably more robotic and
    flat. Runs through the backend (not called directly from the
    browser) purely to keep SARVAM_API_KEY off the client.

    Returns base64-encoded WAV audio that the frontend plays directly;
    if Sarvam isn't configured or the call fails, the frontend catches
    that and falls back to browser speech synthesis automatically —
    Friday keeps talking either way, just less impressively.
    """
    if not SARVAM_API_KEY:
        raise HTTPException(503, "Sarvam TTS is not configured (SARVAM_API_KEY missing)")

    text = payload.text.strip()
    if not text:
        raise HTTPException(400, "text is required")
    text = text[:2500]  # bulbul:v3's hard limit per request
    ...
    try:
        ...
        resp = httpx.post(
            ...
            json={
                ...
                "pace": max(0.5, min(2.0, payload.pace or 1.05)),
                "temperature": 0.8,  # a bit more expressive/enthusiastic than the flat default
            },
            timeout=20,
        )
        ...
        return {"audioBase64": audios[0], "format": "wav"}
    except HTTPException:
        raise
    except httpx.HTTPStatusError as e:
        raise HTTPException(e.response.status_code, f"Sarvam TTS error: {e.response.text[:300]}")
    except Exception as e:
        raise HTTPException(502, f"Sarvam TTS request failed: {e}")
